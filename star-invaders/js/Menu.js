...
class Menu {
  ...
  createMenuLevels(menuContainer) {
    let menuLevelContainerElement = this.createElement('div', 'menu-level-container');
    menuContainer.appendChild(menuLevelContainerElement);

    let menuLevelTextElement = this.createElement('div', 'menu-level-text', 'Levels');
    menuLevelContainerElement.appendChild(menuLevelTextElement);

    let menuLevelsElement = this.createElement('ul', 'menu-levels');
    menuLevelContainerElement.appendChild(menuLevelsElement);

    for (let level = 1; level <= NO_OF_LEVELS; level++) {
      let menuLevelElement = this.createElement('li');
      menuLevelsElement.appendChild(menuLevelElement);

      let menuLevelNameElement = this.createElement('span', 'menu-level-name', 'Level ' + level);
      menuLevelElement.appendChild(menuLevelNameElement);

      let menuLevelBtnElement = this.createElement('button', 'menu-level-btn', 'Start Level');
      menuLevelBtnElement.setAttribute('data-level', level);
      menuLevelElement.appendChild(menuLevelBtnElement);

      let menuBossRoundBtnElement = this.createElement('button', 'menu-boss-round-btn', 'Start Boss Round');
